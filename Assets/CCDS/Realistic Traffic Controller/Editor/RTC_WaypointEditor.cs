    ...
    public override void OnInspectorGUI() {
        ...
        if (Selection.objects.Length <= 1) {
            ...
            if (listCloserWaypoints) {
                ...
                RTC_Waypoint[] waypoints = FindObjectsOfType<RTC_Waypoint>(true);
                ...
                for (int i = 0; i < waypoints.Length; i++) {
                    ...
                    if (!waypoints[i].ignoreGizmos && waypoints[i] != prop && !waypointBehind && Vector3.Distance(waypoints[i].transform.position, prop.transform.position) <= 80f)
