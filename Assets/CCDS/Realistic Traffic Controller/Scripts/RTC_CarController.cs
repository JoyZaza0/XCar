...
public class RTC_CarController : RTC_Core {
    ...
    private readonly float checkPerMilisecond = .5f;
    private float lastUpdateTimer = 0f;

    private void Awake() {

        lastUpdateTimer = checkPerMilisecond;
    ...
    private void Update() {
        ...
        if (lastUpdateTimer >= checkPerMilisecond) {
    ...
    private void Interaction() {
        ...
        RTC_SceneManager sceneManager = RTCSceneManager;
        RTC_CarController[] allVehicles = sceneManager.allVehicles;
        ...
        if (allVehicles != null) {
            ...
            for (int i = 0; i < allVehicles.Length; i++) {
                ...
                if (allVehicles[i] != null) {
                    ...
                    if (Vector3.Distance(transform.position, allVehicles[i].transform.position) < 15f && allVehicles[i] != this && allVehicles[i].gameObject.activeSelf && Vector3.Dot((allVehicles[i].transform.position - transform.position).normalized, transform.forward) > 0)
                        closerVehicles.Add(allVehicles[i]);
